...
class AdultosController extends Controller
{
    ...
    public function edit(Adulto $adulto){
    ...
    public function update(Adulto $adulto, AdultoRequest $request){
        ...
        if ($request->hasFile('foto')) {
            ...
            // salva em storage/app/public/fotos/adultos
            $arquivoPath = $file->storeAs($pasta, $nomeArquivo, 'public');
            $adulto->foto = $arquivoPath; // atualiza o campo da foto
        }
        ...
        return redirect(route('adultos.index'))->with('msg', 'Inscrição atualizada com sucesso!');
    }
